...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (borrow_date && !isRealDate(borrow_date)) {
    ...
    const safeBorrowDate = borrow_date || today();
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (return_date && !isRealDate(return_date)) {
    ...
    const safeReturnDate = return_date || today();
    if (safeReturnDate < existing.borrow_date) {
      return res.status(400).json({ message: 'Return date cannot be earlier than borrow date.' });
    }

    await db.run('UPDATE borrowings SET return_date = ? WHERE id = ?', [safeReturnDate, Number(id)]);
